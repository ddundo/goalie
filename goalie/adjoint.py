                ...
                for j, block in enumerate(reversed(solve_blocks[::-stride])):
                    ...
                    if not self.steady:
                        ...
                        if self.field_types[field] == "unsteady":
                            ...
                            if get_adj_values and dep.adj_value is not None:
                        ...
                        if self.field_types[field] == "steady":
                            block_idx = solve_blocks.index(block)
                            if block_idx == 0:
                                f_idx = self.fields.index(field)
                                adj_value = self._controls[
                                    f_idx
                                ].block_variable.adj_value
                                forward_old = checkpoints[i][field]
                                # adj_value = checkpoints[i][field].block_variable.adj_value
                            else:
                                old_block = solve_blocks[solve_blocks.index(block) - 1]
                                old_out = self._output(field, i, old_block)
                                forward_old = old_out.saved_output
                                adj_value = old_out.adj_value
                            solutions.forward_old[i][j].assign(forward_old)
                            if get_adj_values:
                                if adj_value is not None:
                                    solutions.adj_value[i][j].assign(adj_value)
